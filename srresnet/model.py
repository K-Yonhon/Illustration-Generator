...
def pixel_shuffler(out_ch, x, r = 2):
    b, c, w, h = x.shape
    x = x.reshape(b, r, r, int(out_ch/(r*2)), w, h)
    x = x.transpose(0,3,4,1,5,2)
    out_map = x.reshape(b, int(out_ch/(r*2)), w*r, h*r)
...
class Generator(Chain):
    def __init__(self, base = 64):
        ...
        with self.init_scope():
            ...
            self.l0 = L.Linear(100, 64*16*16, initialW = w)
    ...
    def __call__(self,x):
        h1 = F.reshape(F.relu(self.bn0(self.l0(x))), (100, 64, 16,16))
        h = self.r0(h1)
        h = self.r1(h)
        h = self.r2(h)
