...
parser.add_argument('--iter', default=10000, type=int, help="the number of iterations")
...
dir_list = ['sum_0', 'sum_1', 'sum_2', 'sum_3', 'sum_4', 'sum_5', 'sum_6', 'sum_7']
...
ztest = chainer.as_variable(xp.random.uniform(-1, 1, (batchsize, 256)).astype(xp.float32))
...
for epoch in range(epochs):
    ...
    for batch in range(0, iterations, batchsize):
        ...
        for _ in range(n_dis):
            ...
            z = chainer.as_variable(xp.random.uniform(-1, 1, (batchsize, 256)).astype(xp.float32))
            x, seg = generator(z)
            fake = discriminator(x, seg)
            real = discriminator(t, s)
            dis_loss = loss_hinge_dis(fake, real)
